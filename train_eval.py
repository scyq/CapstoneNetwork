...
def train_by_batch(sos, opt, data, encoder_optimizer, decoder_optimizer,
    ...
    ) < opt.teacher_forcing_ratio else False
    '''
    一次处理一个时刻，即一个字符
    ...
    if use_teacher_forcing:
        for t in range(max_target_length):
            ...
            decoder_output: [batch_size, voc_length]
            '''
            mask_loss, nTotal = maskNLLLoss(decoder_output, targets[t],
                                            mask[t])
        ...
        for t in range(max_target_length):
            ...
            decoder_input = torch.LongTensor(
                [[topi[i][0] for i in range(opt.batch_size)]])
            decoder_input = decoder_input.to(opt.device)
            # 计算累计的loss
            mask_loss, nTotal = maskNLLLoss(decoder_output, targets[t],
                                            mask[t])
...
def train(**kwargs):
    ...
    opt = Config()
    print(f"Now we are using device: {opt.device}")
...
def generate(input_seq, searcher, sos, eos, opt):
    ...
    input_lengths = input_lengths.to(opt.device)
    tokens, scores = searcher(sos, eos, input_batch, input_lengths,
                              opt.max_generate_length, opt.device)
    return tokens
